...
export class UpdateItemField_Transaction extends jsTPS_Transaction { 
    constructor(_id, sortBy, undoSortBy,oldList, sortByVar){
        super();
        this._id = _id;
        this.sortBy = sortBy;
        this.undoSortBy = undoSortBy;
        this.oldList = oldList;
        this.sortByVar = sortByVar;
    }
    async doTransaction() {
        const {data} = await this.sortBy({ variables: {_id: this._id, _sortByVar: this.sortByVar}});  
        return data;
    }
    async undoTransaction(){
        const data  = await this.undoSortBy({ variables: {_id: this._id, _oldList: this.oldList }});
		return data;
    }
}
...
export class jsTPS {
     ...
     */
     async doTransaction() {
        ...
        console.log("UndoTransactions:" + this.hasTransactionToUndo());
        console.log("RedoTransactions:" + this.hasTransactionToRedo());
		console.log(' ')
		return retVal;
    }
