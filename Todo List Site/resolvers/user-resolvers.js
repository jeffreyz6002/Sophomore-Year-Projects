	...
	Mutation: {
		...
		**/
		...
		updateaccount: async (_, args, { res }) => {
			const { email, password, name } = args;
			const alreadyRegistered = await User.findOne({email: email});
			const previousRegistered = await User.findOne({email: name});
			const hashed = await bcrypt.hash(password, 10);
			...
			const saved = await User.updateOne({email: name},{email: email});			// After registering the user, their tokens are generated here so they
			// are automatically logged in on account creation.
			const saved2 = await User.updateOne({email: email},{name: name});
