...
module.exports = {
	...
	Mutation: {
		...
		**/
		...
		sortListBy: async (_, args,) =>{
			const {_id, sortByVar} = args;
			const listId = new ObjectId(_id);
			const found = await Todolist.findOne({_id: listId});
			let listItems = found.items;
			let firstItem = listItems[0];

			if(sortByVar === "description"){
			for(let i = 0; i < listItems.length - 1; i++){
				for(let j = 0; j < listItems.length - 1 - i; j++){
					if(listItems[j].description < listItems[j+1].description){
			...
			if(sortByVar === "due_date"){
				for(let i = 0; i < listItems.length - 1; i++){
					for(let j = 0; j < listItems.length - 1 - i; j++){
						if(listItems[j].due_date < listItems[j+1].due_date){
							let temp = listItems[j];
							listItems[j] = listItems[j+1];
							listItems[j+1] = temp;
						}
			...
			if(sortByVar === "completed"){
				for(let i = 0; i < listItems.length - 1; i++){
					for(let j = 0; j < listItems.length - 1 - i; j++){
						if(listItems[j].completed < listItems[j+1].completed){
							let temp = listItems[j];
							listItems[j] = listItems[j+1];
							listItems[j+1] = temp;
						}
			...
			if(sortByVar === "assigned_to"){
				for(let i = 0; i < listItems.length - 1; i++){
					for(let j = 0; j < listItems.length - 1 - i; j++){
						if(listItems[j].assigned_to < listItems[j+1].assigned_to){
							let temp = listItems[j];
							listItems[j] = listItems[j+1];
							listItems[j+1] = temp;
						}
		...
		undoSortListBy: async (_, args,) =>{
			const {_id, _oldList} = args;
			const updated = await Todolist.updateOne({_id: _id}, { items: _oldList })
			if(updated) return (_oldList);
		} 
	}
}